...
async function getGoldRate() {
  ...
  try {
    ...
    // Use ID to find the element first, then select class within that ID
    const rate = await page.$eval('#show-rate-block .price.22kt-price', el => el.textContent.trim());
    const date = await page.$eval('#show-rate-block .date.update-date', el => el.textContent.trim());
